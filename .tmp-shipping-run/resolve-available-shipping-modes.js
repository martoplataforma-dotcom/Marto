...
exports.resolveAvailableShippingModes = resolveAvailableShippingModes;
const analyze_shipping_profile_1 = require("./analyze-shipping-profile");
function pushBlocked(blockedModes, mode, reason) {
    blockedModes.push({ mode, reason });
}
function pickSuggestedPrimaryShippingMode(availableShippingModes, input) {
    if (availableShippingModes.length === 0) {
        return null;
    }
    if (input.analysis.shouldPrioritizeTransportadora &&
        availableShippingModes.includes('TRANSPORTADORA')) {
        return 'TRANSPORTADORA';
    }
    if (input.analysis.canUseCorreios &&
...
function resolveAvailableShippingModes(input) {
    ...
    const availableShippingModes = [];
    const blockedModes = [];
    const hasPhysicalData = (0, analyze_shipping_profile_1.hasCompletePhysicalData)({
    ...
    // CORREIOS
    if (!hasPhysicalData) {
        pushBlocked(blockedModes, 'CORREIOS', 'Dados logísticos do produto incompletos.');
    }
    else if (!input.analysis.canUseCorreios) {
        pushBlocked(blockedModes, 'CORREIOS', 'Perfil logístico incompatível com Correios.');
    }
    else if (!input.permissions.allowCorreios) {
        pushBlocked(blockedModes, 'CORREIOS', 'Produto não permite envio por Correios.');
    }
    ...
    // TRANSPORTADORA
    if (!hasPhysicalData) {
        pushBlocked(blockedModes, 'TRANSPORTADORA', 'Dados logísticos do produto incompletos.');
    }
    else if (!input.permissions.allowTransportadora) {
        pushBlocked(blockedModes, 'TRANSPORTADORA', 'Produto não permite envio por transportadora.');
    }
    else if (!input.capabilities.supportsTransportadora) {
        pushBlocked(blockedModes, 'TRANSPORTADORA', 'Vendedor não opera com transportadora.');
    }
    else {
    ...
    if (!input.permissions.allowLocalDelivery) {
        pushBlocked(blockedModes, 'LOCAL_DELIVERY', 'Produto não permite entrega local.');
    }
    else if (!input.capabilities.supportsLocalDelivery) {
        pushBlocked(blockedModes, 'LOCAL_DELIVERY', 'Vendedor não oferece entrega local.');
    }
    else {
    ...
    return {
        ...
        availableShippingModes,
        blockedModes,
        suggestedPrimaryShippingMode: pickSuggestedPrimaryShippingMode(availableShippingModes, input),
    };
}
